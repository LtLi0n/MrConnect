...
namespace DataServerHelpers
{
    public class SocketModuleBase<T> : BetterModuleBase<T> where T : CustomCommandContext
    {
        ...
        public void Reply<EntityT, PkType>(string message, IEntity<EntityT, PkType> entity) => 
            Context.User.ReplyMessage(Context.Packet, Context.Header, message, entity.Id);
    }
}
